#include "mixer-10m-1m.hpp"

Model* model_Mixer10m1m = createModel<Mixer10m1m, Mixer10m1m_Widget>("Mixer-10m-1m");

Mixer10m1m::Mixer10m1m() {
...
Mixer10m1m_Widget::Mixer10m1m_Widget(Mixer10m1m* module) {
	setModule(module);
	setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/panels/mixer-10m-1m.svg")));
	...
	addParam(createParamCentered<AHMRoundKnobWhiteTiny>(mm2px(Vec(6.820, 13.472)), module, Mixer10m1m::KNOB_MAINCV_PARAM));
	addInput(createInputCentered<PJ301MPort>(mm2px(Vec(6.82, 22.498)), module, Mixer10m1m::JACK_MAINCV_INPUT));

	addParam(createParamCentered<Mixer10m1m::MixerMainLevelKnob>(mm2px(Vec(20.32, 18.0)), module, Mixer10m1m::KNOB_MAINLEVEL_PARAM));
	addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(33.82, 18.0)), module, Mixer10m1m::JACK_MAIN_OUTPUT));

	for (int track = 0; track < TRACKS_NUMBER; track++) {
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(6.865, 115.061 - 9 * track)), module, Mixer10m1m::JACK_IN_INPUT + track));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(15.865, 115.061 - 9 * track)), module, Mixer10m1m::JACK_CV_INPUT + track));
		addParam(createParamCentered<AHMRoundKnobWhiteTiny>(mm2px(Vec(24.865, 115.061 - 9 * track)), module, Mixer10m1m::FADER_LEVEL_PARAM + track));
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(33.865,  115.061 - 9 * track)), module, Mixer10m1m::JACK_OUT_OUTPUT + track));
	}
}

void Mixer10m1m::process(const ProcessArgs& args) {
	int voices_needed = 1;
		...
		if (outputs[JACK_MAIN_OUTPUT].isConnected()) {
			for (int track = 0; track < TRACKS_NUMBER; track++) {
				...
				if (inputs[JACK_MAINCV_INPUT].isConnected()) {
					float main_cv = clamp(inputs[JACK_MAINCV_INPUT].getPolyVoltage(voice) / 10.f, 0.f, 1.f) * main_cv_param;
					main_value[voice] *= main_cv;
				}
			}
...
float Mixer10m1m::compensateVolume(int current_track_number, float fader_level[]) {
	float value = 0.f;
	for (int track = 1; track < TRACKS_NUMBER; track++) {
		...
		if (inputs[JACK_IN_INPUT + track_number].isConnected())
...
Mixer10m1m::MixerMainLevelKnob::MixerMainLevelKnob() {
...
Mixer10m1m::MixerLevel::MixerLevel() {
